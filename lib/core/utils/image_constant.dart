class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';

  // Splash images
  static String imgIllustratedFri = '$imagePath/img_illustrated_fri.png';

  // Login images
  static String imgLogoooooooooooo = '$imagePath/img_logoooooooooooo.png';

  static String imgImage1 = '$imagePath/img_image_1.png';

  // Map images
  static String imgImage12 = '$imagePath/img_image_12.png';

  static String imgImage11 = '$imagePath/img_image_11.png';

  static String imgImage21 = '$imagePath/img_image_21.png';

  // Profile images
  static String imgEllipse1 = '$imagePath/img_ellipse_1.png';

  static String imgEllipse2 = '$imagePath/img_ellipse_2.png';

  static String imgHighSchoolBui = '$imagePath/img_high_school_bui.png';

  static String imgPngtreeVector = '$imagePath/img_pngtree_vector.png';

  static String imgImage13 = '$imagePath/img_image_13.png';

  static String imgImage15 = '$imagePath/img_image_15.png';

  static String img46d9985ea3beb17 = '$imagePath/img_46d9985ea3beb17.png';

  // Common images
  static String imgImage3 = '$imagePath/img_image_3.png';

  static String imgBjnkjhugycRemovebgPreview =
      '$imagePath/img_bjnkjhugyc_removebg_preview.png';

  static String imgRectangle10 = '$imagePath/img_rectangle_10.png';

  static String imgImage4 = '$imagePath/img_image_4.png';

  static String imgImage6 = '$imagePath/img_image_6.png';

  static String imgImage16 = '$imagePath/img_image_16.png';

  static String imgImage19 = '$imagePath/img_image_19.png';

  static String imgImage18 = '$imagePath/img_image_18.png';

  static String imgImage22 = '$imagePath/img_image_22.png';

  static String imgImage7 = '$imagePath/img_image_7.png';

  static String imageNotFound = 'assets/images/image_not_found.png';
}
